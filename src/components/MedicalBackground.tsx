...
export function MedicalBackground() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none bg-background noise">
      {/* Animated grid */}
      <div className="absolute inset-0 opacity-40" style={{ backgroundImage: "linear-gradient(var(--color-primary) 1px, transparent 1px), linear-gradient(90deg, var(--color-primary) 1px, transparent 1px)", backgroundSize: "40px 40px", opacity: 0.05 }} />

      {/* Medical neon glow following the pointer */}
      <motion.div
        className="absolute inset-0 opacity-70"
        style={{
          background: `radial-gradient(circle at ${glowX} ${glowY}, oklch(0.85 0.1 200 / 0.3), transparent 34%)`,
        }}
      />
      ...
      <motion.div
        ...
        style={{ background: "radial-gradient(circle, oklch(0.8 0.15 190), transparent)" }}
      ...
      {Array.from({ length: 34 }).map((_, i) => (
        <motion.div
          key={i}
          className="absolute w-1 h-1 rounded-full bg-primary/60 shadow-lg"
      ...
      <div className="absolute top-[55%] -translate-y-1/2 left-0 w-[200%] opacity-40 overflow-hidden pointer-events-none">
        <motion.svg
